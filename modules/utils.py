...
def calculate_lpips(img1, img2, lpips_model):
    img1 = img1.detach() * 2 - 1
    img2 = img2.detach() * 2 - 1
    with torch.no_grad():  # 加上避免计算图构建
...
def calculate_psnr(a: torch.Tensor, b: torch.Tensor, eps=1e-10) -> float:
    """在[0,1]空间计算PSNR"""
    # a = denorm01(a)
    # b = denorm01(b)
...
def calculate_ssim(img1, img2):
    ...
    for i in range(img1.shape[0]):
        ssim_val += ssim(img1[i], img2[i], channel_axis=-1, data_range=1.0)
